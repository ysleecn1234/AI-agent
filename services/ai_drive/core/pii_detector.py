...
class PIIDetector:
    ...
    def __init__(self):
        ...
        self.patterns = {
            ...
            "주소": r'(서울|부산|대구|인천|광주|대전|울산|세종|경기|강원|충북|충남|전북|전남|경북|경남|제주)(특별시|광역시|특별자치시|도)?\s?[\w]+[시군구](\s?[\w]+[동읍면리])?',
        }
    ...
    def partial_mask(self, text: str, enabled_items: Dict[str, bool] = None) -> str:
        ...
        if "주소" in active_types:
            masked_text = re.sub(
                r'(서울|부산|대구|인천|광주|대전|울산|세종|경기|강원|충북|충남|전북|전남|경북|경남|제주)(특별시|광역시|특별자치시|도)?\s?[\w]+[시군구](\s?[\w]+[동읍면리])?',
                lambda m: m.group(1) + ('특별시' if '특별시' in m.group() else '광역시' if '광역시' in m.group() else m.group(2) or '') + ' ***',
                masked_text
            )
